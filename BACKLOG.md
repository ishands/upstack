...
- [ ] Sharpen "The Work Doer" anti-pattern in `core/meta/ANTI-PATTERNS.md` — the current definition ("producing the learner's deliverable") doesn't distinguish between two distinct acts: (1) providing reference knowledge and (2) composing the learner's work. Reference knowledge — syntax patterns, templates, formulas, structural rules — is always permitted and should be given directly when a learner is blocked. Composing the learner's work is never permitted, even when it uses only vocabulary you just provided. The rule: give the minimum skeleton (pattern, not solution), then return the work to the learner. Examples to include: coding — give `try: ... except ValueError as e: ...` as a skeleton, never write the full function; non-coding — give the four-section report template, never write the status report. Add a corresponding operational rule to `core/meta/TUTOR-CONTRACT.md` §2 (Socratic Protocol) or §4 (productive struggle): when a learner is blocked on reference knowledge, give the pattern, not the solution, then hand back.
- [ ] Add learner pushback handling to `core/meta/TUTOR-CONTRACT.md` — when a learner expresses frustration or pushback ("you're asking me to write code I don't know"), the tutor must: (1) assess whether the frustration is legitimate (blocked on reference knowledge not yet introduced → give the pattern directly) or premature (hasn't attempted yet → reframe and hint); (2) correct course accordingly; (3) never treat pushback as authorisation to hand over the composed answer. Capitulating to social pressure is a contract violation distinct from, and not justified by, the learner's fair point.
